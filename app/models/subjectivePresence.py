# -*- coding: utf-8 -*-
"""
Presença do aluno numa avaliação subjetiva (Test.evaluation_mode == 'subjective').
...
class SubjectivePresence(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('test_id', 'student_id', name='uq_subjective_presence_test_student'),
        {"schema": "tenant"},
    )
    ...
    def __init__(self, test_id, student_id, present=True, updated_by=None, **kwargs):
    ...
    def __repr__(self):
        return f'<SubjectivePresence {self.id}: Test {self.test_id}, Student {self.student_id} = {self.present}>'
