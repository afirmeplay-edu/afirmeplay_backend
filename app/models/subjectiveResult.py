# -*- coding: utf-8 -*-
"""
Rubrica de correção manual da avaliação subjetiva (Test.evaluation_mode == 'subjective').
...
class SubjectiveResult(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            'test_id', 'question_id', 'student_id',
    ...
    def __init__(self, test_id, question_id, student_id, value, corrected_by=None, **kwargs):
    ...
    def __repr__(self):
        return f'<SubjectiveResult {self.id}: Test {self.test_id}, Question {self.question_id}, Student {self.student_id} = {self.value}>'
