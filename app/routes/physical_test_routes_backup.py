...
@bp.route('/batch-correction/stream/<string:job_id>')
@jwt_required()
@role_required("admin", "professor", "coordenador", "diretor")
def stream_batch_correction_progress(job_id):
    """
    Stream de progresso da correção em lote usando Server-Sent Events (SSE)
    
    Args:
        job_id: ID do job de correção em lote
        
    Returns:
        Server-Sent Events stream com progresso em tempo real
    """
    try:
        from app.services.batch_correction_service import batch_correction_service
        ...
        def generate_progress():
            ...
            try:
                ...
                while True:
                    ...
                    job_status = batch_correction_service.get_job_status(job_id)
                    ...
                    # Se job terminou, enviar resultado final e sair
                    if job_status['status'] in ['completed', 'failed', 'cancelled']:
                        # Buscar resultados finais
                        final_results = batch_correction_service.get_job_results(job_id)
                        if final_results:
                            final_data = {
                                'type': 'completed',
                                'job_id': job_id,
                ...
                logging.error(f"Erro no stream SSE do job {job_id}: {str(e)}")
        ...
        logging.error(f"Erro ao criar stream SSE: {str(e)}")
        return jsonify({"error": "Erro ao criar stream de progresso"}), 500


@bp.route('/batch-correction/status/<string:job_id>', methods=['GET'])
@jwt_required()
@role_required("admin", "professor", "coordenador", "diretor")
def get_batch_correction_status(job_id):
    """
    Obtém status atual de um job de correção em lote
    ...
    try:
        ...
        logging.error(f"Erro ao obter status do job {job_id}: {str(e)}")
        return jsonify({"error": "Erro interno do servidor"}), 500


@bp.route('/batch-correction/results/<string:job_id>', methods=['GET'])
@jwt_required()
@role_required("admin", "professor", "coordenador", "diretor")
def get_batch_correction_results(job_id):
    """
    Obtém resultados finais de um job de correção em lote
    
    Args:
        job_id: ID do job
        
    Returns:
        Resultados finais do job
    """
    try:
        from app.services.batch_correction_service import batch_correction_service
        
        results = batch_correction_service.get_job_results(job_id)
        if not results:
            return jsonify({"error": "Job não encontrado ou ainda em processamento"}), 404
        ...
        logging.error(f"Erro ao obter resultados do job {job_id}: {str(e)}")
        return jsonify({"error": "Erro interno do servidor"}), 500


@bp.route('/batch-correction/cancel/<string:job_id>', methods=['POST'])
@jwt_required()
@role_required("admin", "professor", "coordenador", "diretor")
def cancel_batch_correction(job_id):
    """
    Cancela um job de correção em lote em andamento
    ...
    try:
        ...
        success = batch_correction_service.cancel_job(job_id)
        if not success:
            return jsonify({"error": "Job não encontrado ou não pode ser cancelado"}), 400
...
@bp.route('/batch-correction/cleanup', methods=['POST'])
@jwt_required()
@role_required("admin", "tecadm")
def cleanup_old_batch_jobs():
    """
    Remove jobs antigos do banco de dados (apenas para administradores)
    
    Body (JSON):
    {
        "hours": 24 // Idade em horas para considerar job como antigo
    }
    ...
    try:
        ...
        batch_correction_service.cleanup_old_jobs(hours)
        
        return jsonify({
            "message": f"Jobs antigos (>{hours}h) removidos com sucesso"
        }), 200
