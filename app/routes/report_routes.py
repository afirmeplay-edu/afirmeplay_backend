...
def relatorio_pdf(evaluation_id: str):
    ...
    try:
        ...
        # Gerar relatório PDF usando WeasyPrint com template HTML moderno
        try:
            from weasyprint import HTML
            from jinja2 import Environment, FileSystemLoader, select_autoescape
            import math
            ...
            metadados = {"scope_type": scope_type, "scope_id": scope_id, "mes": mes_atual, "ano": ano_atual, "data_geracao": data_geracao}
            try:
            ...
            # Configurar Jinja2 e carregar template
...
def _montar_resposta_relatorio(evaluation_id: str, school_id: str = None, city_id: str = None) -> dict:
    ...
    avaliacao_data = {
        ...
        "course_name": getattr(test, 'course_name', None) or "",
        "disciplinas": _obter_disciplinas_avaliacao(test)
    }
    ...
    return {
        ...
        "avaliacao": {
            "id": avaliacao_data["id"],
            "titulo": avaliacao_data["titulo"],
            "descricao": avaliacao_data.get("descricao"),
            "course_name": avaliacao_data.get("course_name", ""),
            "disciplinas": avaliacao_data.get("disciplinas", [])
        },
        "metadados": metadados,
        "niveis_aprendizagem": niveis_aprendizagem,
        "nota_geral": nota_geral,
...
def dados_json(evaluation_id: str):
    try:
        school_id = request.args.get('school_id')
        city_id = request.args.get('city_id')
        resposta = _montar_resposta_relatorio(evaluation_id, school_id, city_id)
...
@bp.route('/relatorio-weasy/<string:evaluation_id>', methods=['GET'])
@jwt_required()
@role_required("admin", "professor", "coordenador", "diretor","tecadm")
def relatorio_weasy(evaluation_id: str):
    try:
        school_id = request.args.get('school_id')
        city_id = request.args.get('city_id')
        ...
        def sum_values(items, attr='value'):
            """Soma os valores de um atributo em uma lista de dicts"""
            try:
                return sum(item.get(attr, 0) if isinstance(item, dict) else getattr(item, attr, 0) for item in items)
            except (TypeError, AttributeError):
                return 0
        ...
        def cos_filter(value):
            """Calcula o cosseno de um valor em radianos"""
            try:
                return math.cos(float(value))
            except (TypeError, ValueError):
                return 0
        ...
        def sin_filter(value):
            """Calcula o seno de um valor em radianos"""
            try:
                return math.sin(float(value))
            except (TypeError, ValueError):
                return 0
        ...
        from flask import Response
        test = Test.query.get(evaluation_id)
        nome_avaliacao = (test.title if test else 'relatorio').replace(' ', '_').replace('/', '_').replace('\\', '_').replace(':', '_').replace('?', '_').replace('*', '_').replace('"', '_').replace('<', '_').replace('>', '_').replace('|', '_')
        nome_arquivo = f"relatorio_{nome_avaliacao}.pdf"
        response = Response(pdf_bytes, mimetype='application/pdf')
        response.headers['Content-Disposition'] = f'attachment; filename={nome_arquivo}'
        return response
        ...
        import traceback
        traceback_str = traceback.format_exc()
        logging.error(f"Erro ao gerar relatório WeasyPrint: {str(e)}\n{traceback_str}")
        return jsonify({"error": "Erro ao gerar relatório PDF (WeasyPrint)"}), 500
