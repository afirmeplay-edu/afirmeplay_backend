...
@jwt_required()
@role_required("student", "admin", "professor")
def start_test_session():
    """
    Inicia uma nova sessão de prova para um aluno
    ...
    try:
        ...
        if not student_id or not test_id:
            return jsonify({'error': 'student_id e test_id são obrigatórios'}), 400
        
        # Verificar se estudante e teste existem
...
def get_student_sessions(student_id):
    """
    Retorna todas as sessões de um aluno
    """
    try:
        student = Student.query.get(student_id)
        if not student:
...
def submit_answers_legacy():
    ...
    try:
        ...
        if not student_id or not test_id or not answers:
            return jsonify({'error': 'student_id, test_id e answers são obrigatórios'}), 400
