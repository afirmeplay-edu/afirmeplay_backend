...
def paper90(im_color):
    ...
    """
    print(f"🔍 [DEBUG paper90] Iniciando orientação da página...")
    print(f"🔍 [DEBUG paper90] Dimensões da imagem de entrada: {im_color.shape if im_color is not None else 'None'}")
    ...
    print(f"🔍 [DEBUG paper90] Dimensões originais: {height}x{width}")
    if min(width, height) > 2700:
        print(f"🔍 [DEBUG paper90] Redimensionando (muito grande): {min(width, height)} > 2700")
        im_color = resizeImg(im_color, 2700)
    elif min(width, height) < 1260:
        print(f"🔍 [DEBUG paper90] Redimensionando (muito pequeno): {min(width, height)} < 1260")
        im_color = resizeImg(im_color, 1260)

    height, width = im_color.shape[:2]
    print(f"🔍 [DEBUG paper90] Dimensões após redimensionamento: {height}x{width}")
    print(f"🔍 [DEBUG paper90] Buscando triângulos (getInfoTriang)...")
    triangles, im_gray, im_bw = getInfoTriang(im_color)
    print(f"🔍 [DEBUG paper90] Triângulos encontrados: {len(triangles)}")

    #Verificar de existem 4 triângulos de interesse
    if len(triangles) < 4:
        print(f"❌ [DEBUG paper90] ERRO: Apenas {len(triangles)} triângulos encontrados, esperado 4")
        return None
    
    print(f"✅ [DEBUG paper90] 4 triângulos encontrados, processando...")
    ang_1 = []
    for i in triangles:
    ...
    print(f"🔍 [DEBUG paper90] Ângulo calculado: {angulo}°")
    ...
    print(f"🔍 [DEBUG paper90] Imagem rotacionada: {rotated_img.shape}")
    #Salva imagens
    #storeImg(im_color, rotated_img)


    triangles = getInfoTriang(rotated_img)[0]
    print(f"🔍 [DEBUG paper90] Triângulos após primeira rotação: {len(triangles)}")
    ...
    if len(sec) == 4:
        print(f"🔍 [DEBUG paper90] Aplicando segunda correção de ângulo...")
        if (abs(getProportion(sec[2],sec[3])[1]) > 0.01) or abs(ang_1[0]-ang_1[3]/max(ang_1[0], ang_1[3])) > 0.035:
            ...
            print(f"🔍 [DEBUG paper90] Segunda correção aplicada: {rotated_img.shape}")
    else:
        print(f"⚠️ [DEBUG paper90] Apenas {len(sec)} triângulos encontrados após primeira rotação (esperado 4), pulando segunda correção")

    print(f"✅ [DEBUG paper90] Orientação concluída: {rotated_img.shape}")
    return rotated_img
